# ITIL-v4-Service-Desk-Design
ITIL v4 IT service management framework for a simulated Ontario SME


![ITIL v4](https://img.shields.io/badge/ITIL-v4%20Foundation-blue)
![Status](https://img.shields.io/badge/Status-Complete-brightgreen)
![Deliverables](https://img.shields.io/badge/Deliverables-9-orange)

---

### Overview

This repository simulates a real-world IT service desk environment for Richmond Solutions Inc., a fictional 150-user professional services organization based in Ontario, Canada.

The project demonstrates practical IT support and IT service management operations aligned with modern ITSM practices, including:

- Incident management
- Service request fulfilment
- SLA tracking
- Change management
- Problem management
- Continual improvement
- Operational troubleshooting workflows
- Knowledge base development
- User onboarding procedures

The repository is designed to reflect realistic SMB IT operations rather than enterprise-scale infrastructure.

---

### Simulated Organization

| Detail | Information |
|:-------|:------------|
| **Company** | Richmond Solutions Inc. |
| **Industry** | Professional Services |
| **Location** | Richmond Hill, Ontario, Canada |
| **Users Supported** | ~150 employees |
| **IT Team** | 3-person IT department (1 IT Manager, 2 Support Analysts) |
| **Challenge** | No formal ITSM structure — incidents, requests, and changes handled informally |

## 

### Operational Environment


Core Technologies

- Microsoft 365
- Microsoft Teams
- Microsoft Entra ID
- Active Directory
- Windows 11 endpoints
- VPN remote access
- Shared drives and file services
- Network printers
- SharePoint-based documentation
- Simulated ITSM ticketing workflows

##

### Repository Structure

| # | Document | Purpose | ITIL Practices Applied |
|---|:---------|:--------|:-----------------------|
| 01 | [Incident-Management](./01-Incident-Management/incident-management-process.md) | Incident response lifecycle, prioritization, escalation | Incident Management, Service Desk, Monitoring & Event Management |
| 02 | [Service-Request-Catalogue](./02-Service-Request-Catalogue/service-request-catalogue.md) | Standardized user request workflows | Service Request Management, Service Desk, Service Level Management |
| 03 | [SLA-&-OLA-Document](./03-SLA-OLA/sla-ola-document.md) | Service level targets and operational commitments | Service Level Management, Relationship Management |
| 04 | [Problem-Management-Register](./04-Problem-Management/problem-management-register.md) | Root cause analysis and recurring incident management | Problem Management, Continual Improvement |
| 05 | [Change-Management-Procedure](./05-Change-Management/change-management-procedure.md) | Controlled implementation of IT changes | Change Enablement, Release Management |
| 06 | [Continual-Improvement-Register](./06-Continual-Improvement/continual-improvement-register.md) | Service optimization initiatives and operational metrics | Continual Improvement, Service Level Management |
| 07 | [Sample-Tickets](./07-Sample-Tickets/sample-ticket-catalogue.md) | Realistic sample IT service tickets demonstrating operational workflows | Realistic incident, request, and change ticket examples |
| 08| [Knowledge-Base](./08-Knowledge-Base/knowledge-base.md) | Record accurate  operational knowledge | Troubleshooting documentation and support procedures |
| 09 | [Onboarding-Workflow](./09-Onboarding-Workflow/onboarding-workflow.md) | Standardize and control the onboarding process | New user provisioning and onboarding process |

##

### Key Skills Demonstrated


IT Support Operations
- Incident triage and prioritization
- Ticket lifecycle management
- Troubleshooting documentation
- Escalation handling
- SLA awareness and tracking
- End-user support workflows

##

IT Service Management (ITSM)
- Incident Management
- Service Request Management
- Problem Management
- Change Enablement
- Continual Improvement
- Service Level Management

##

Operational Documentation
- Knowledge base creation
- SOP development
- Process standardization
- Service workflow design
- Root cause analysis documentation

##

Example Operational Scenarios

The repository includes realistic support scenarios such as:

- VPN instability during Teams meetings
- Outlook startup failures
- Shared drive connectivity issues
- Printer outages
- User onboarding workflows
- Password reset and MFA support
- Microsoft 365 administration changes
- Recurring incident trend analysis

## 

### Purpose of This Project

This portfolio project was built to simulate day-to-day responsibilities commonly performed in:
- IT Help Desk
- Service Desk Analyst
- Junior IT Support
- Managed Service Provider (MSP) environments
- IT Operations support teams

The goal is to demonstrate practical operational thinking, structured troubleshooting methodology, and ITSM process understanding within a realistic SMB support environment.

##

### Operational Focus Areas

Incident Handling

Structured response workflows including:
- Detection
- Logging
- Prioritization
- Escalation
- Resolution
- Closure

##

### Service Request Fulfilment

Standardized request workflows for:
- Account provisioning
- Software installation
- Access requests
- Hardware requests
- Onboarding tasks

##

### Change Control

Controlled operational changes including:
- Risk assessment
- Rollback planning
- Implementation scheduling
- Post-implementation review

##

### Continual Improvement

Service optimization initiatives driven by:
- Ticket trends
- SLA performance
- Recurring incidents
- User feedback

##

### Repository Goals

This repository was designed to demonstrate:
- Operational IT support realism
- Process-oriented troubleshooting
- Structured IT documentation
- Practical ITSM implementation
- Communication and escalation workflows

##

### Author

Andrew

Ontario, Canada

Focused on:
- IT Support
- IT Service Management
- Identity & Access Management (IAM)
- Governance, Risk, and Compliance (GRC)
- Cybersecurity operations

## 

### Disclaimer

Richmond Solutions Inc. is a fictional organization created for portfolio and educational purposes. All systems, users, incidents, and operational scenarios are simulated.
